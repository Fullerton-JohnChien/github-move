...
if(!empty($_SESSION["pay_step"])){
	$checked = array("same", "license", "en_driver", "jp_driver", "ct_driver", "ko_driver",
			"baby_set", "child_set", "placard", "female", "accessible", "wifi", "suggest", "policy_chk");
	$selected = array("gender", "living_country_id", "car_gender", "car_living_country_id", "get_on_time");
...
$fleet_route = $tripitta_service->get_fleet_route_detail($cr_type, $fr_id);
$total_adult = $car_adult * $fleet_route['fr_adult_price'];
$total_child = $car_child * $fleet_route['fr_child_price'];
$total_price = $total_adult + $total_child;
$car_id = $fleet_route["c_id"];
$luggage = $tripitta_service->find_car_luggage($car_id, $car_adult+$car_child);
$cr_id = $fleet_route['cr_id'];
...
if(empty($coupon) || $coupon == "undefined") {
	...
	if(!empty($market_list)) {
		foreach ($market_list as $ml) {
			$mc_id = $ml["mc_id"];
			$bo_array = $tripitta_service-> cal_marking_campain_discount($total_price, $mc_id, $_SESSION['travel.ezding.user.data']["serialId"]);
		...
		<div class="pBlock">
			<div class="pbLeft">
				...
				<div class="listBlock">
					...
					<div class="pdata"><span class="black">NTD</span><span class="black"><?php echo number_format($fleet_route['fr_adult_price']); ?></span></div>
					<div class="ptitle1">成人人數</div>
					<div class="pdata"><span class="black"><?php echo $car_adult; ?> 人</span></div>
				</div>
				<div class="listBlock">
					<div class="ptitle">孩童單價</div>
					<div class="pdata"><span class="black">NTD</span><span class="black"><?php echo number_format($fleet_route['fr_child_price']); ?></span></div>
					<div class="ptitle1">孩童人數</div>
					<div class="pdata"><span class="black"><?php echo $car_child; ?> 人</span></div>
				</div>
				<?php
					if (!empty($luggage)) {
						$i = 0;
					...
					?>
				<div class="listBlock">
					<div class="ptitle"><?php if ($i == 0) { echo "行李"; } ?></div>
			...
			<div class="pbRight">
				...
				<div class="listBlock">
					...
					<div class="pNum"><?php echo number_format($total_adult); ?></div>
				</div>
				<div class="listBlock">
					<div class="ptitle">孩童小計</div>
					<div class="pCurrency">NTD</div>
					<div class="pNum"><?php echo number_format($total_child); ?></div></div>
				<div class="listBlock">
					<hr>
				</div>
				<div class="listBlock">
					...
					<div class="pCurrency">NTD</div>
					<div class="pNum"><?php echo number_format($total_price); ?></div>
				</div>
				<!--
				<div class="listBlock" style="display:none;">
...
<section id="step2">
	<!--
	<div class="require sec">
		<h3>您的需求為</h3>
		<div class="rBlock">
			<label class="blank location">
				<input type="text" id="get_on" name="get_on" maxlength="60" placeholder="去程地點（例如：台北金華酒店）">
			</label>
			<label class="blank rTime">
				<select id="get_on_time" name="get_on_time">
					<option value="" disabled="true" selected>去程上車時間</option>
					<?php
						for($i=1;$i<=24;$i++){
							for($j=0;$j<60;$j+=30){
			...
			<label class="blank location">
				<input type="text" id="get_off" name="get_off" maxlength="60" placeholder="回程地點（例如：台北金華酒店）">
			</label>
		</div>
		<label class="checkLabel">
			<div class="checkSignWrap">
				<input type="checkbox" id="suggest" name="suggest" value="1" class="checkSwitch">
				<i class="fa fa-check-square-o" aria-hidden="true"></i>
				<i class="fa fa-square-o" aria-hidden="true"></i>
			</div>
			<div class="textBlock">
				<div>
					我想要走建議行程：<br><?php echo $route_spot; ?>
				</div>
				<div class="onge">
					（打勾即同意車隊為您安排的路線，若您有其他想要加入的景點，請訂購後透過訂單訊息與業者聯繫，若超過    範圍則需加價。）
				</div>
			</div>
		</label>
	</div>
		...
		<div class="passenger">
			...
			<div class="pInfo">
				...
				<label class="blank county">
					<select id="passenger_country[<?= $i ?>]" name="passenger_country[<?= $i ?>]">
...
<section id="step3">
	...
	<div class="sec">
		<h3>亞洲萬里通卡號</h3>
		<label class="blank">
			<input type="text" name="" maxlength="20" placeholder="輸入卡號">
		</label>
	</div>
	-->
	<div class="sec">
		<h3>備註留言</h3>
		<input type="hidden" id="suggest" name="suggest" value="0">
		...
		<label class="checkLabel">
			...
			<div class="textBlock">
				<div>
					我已閱讀並同意<a href="javascript:void(0)" id="popup_policy">訂車條款和取消規定之內容</a>
				</div>
			</div>
		</label>
	</div>
...
<div class="popupChartPolicy">
	<div class="closeBtn" onclick="popup('0');">
...
// 				alert('item:'+$(this).val());
				if($(this+":selected").val()=="" && result=="OK"){
					$(this).focus();
					result = "FAIL";
				}
			});
	...
	$(function(){
        ...
        $('#step1 .pre').on("click", function(){
            var url = "/bookingcar/tourbus/<?php echo $fr_id; ?>/";
            url += "?begin_date=<?php echo $begin_date; ?>&car_adult=<?php echo $car_adult; ?>&car_child=<?php echo $car_child; ?>";
        ...
        $('#user_coupon').click(function () {
            ...
            // 先還原
            $('#bonus').html("-0");
            $('#pay_total').html("NTD <?php echo number_format($total_price); ?><span>(約 NTD <?php echo number_format($total_price); ?>)</span>");
           	// 在折扣
           	// @todo
