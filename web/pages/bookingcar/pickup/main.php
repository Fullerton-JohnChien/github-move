...
<body>
	...
	<main class="pickup-index-container">
		...
		<section class="selector" id="popupSelectList">
			...
			<div class="wrap">
				<div class="stitle">服務類型</div>
				<div class="selectWrap leftRadius">
					<select id="pickup_type" name="pickup_type">
						<option value="2" <?php if ($pickup_type == 2) { echo "selected"; } ?>>接機</option>
						<option value="4" <?php if ($pickup_type == 4) { echo "selected"; } ?>>送機</option>
					</select>
					<i class="fa fa-angle-down"></i>
				</div>
			</div>
			<div class="wrap">
                <div class="stitle">用車日期</div>
		...
		<section class="selector-m">
			...
			<!--<li>
				...
				<li>
					<div>目的地:</div>
					<div id="end_area_mobile"><?php
					if($pickup_type == 2) {
						echo get_area_name_by_id($area_list, $end_area);
					}else if($pickup_type == 4) {
						echo get_area_name_by_id($area_deliver_list, $end_area);
					}
					?></div>
				</li>
				<li>
				...
				<li>
					<div>出發地:</div>
					<div id="begin_area_mobile"><?php
					if($pickup_type == 2) {
						echo get_area_name_by_id($area_deliver_list, $begin_area);
					}else if($pickup_type == 4) {
						echo get_area_name_by_id($area_list, $begin_area);
					}
					?></div>
				</li>
				<li>
			...
			<?php
                ...
                if (!empty($fleet_route_list)) {
                    foreach ($fleet_route_list as $fr) {
                        $url = "/bookingcar/pickup/" . $fr["fr_id"] . "/";
                        $url .= "?pickup_type=" . $pickup_type . "&begin_date=" . $begin_date . "&car_adult=" . $car_adult . "&car_child=" . $car_child;
...
$(function () {
        ...
        query_car = function () {
            ...
            url += '&begin_area=' + encodeURIComponent(begin_area);
        ...
        query_sort = function () {
            var params = '';
            var and = '';
            <?php if (!empty($pickup_type)) { ?>
            var b_pickup_type = $('#popupSelectList #pickup_type :selected').val();
            if (params != "") {
                and = "&";
            }
            params += and + 'pickup_type=' + encodeURIComponent(b_pickup_type);
			<?php } ?>
...
<?php } ?>
	...
	function getresult(pageno) {
        ...
        var car_child = $('#car_child :selected').val();
        var sort = $('#sort :selected').val();
        var url = '/pickup/scroll/?pickup_type=' + encodeURIComponent(pickup_type);
        url += '&begin_area=' + encodeURIComponent(begin_area);
        url += '&end_area=' + encodeURIComponent(end_area);
        url += '&begin_date=' + encodeURIComponent(begin_date);
        url += '&car_day=' + encodeURIComponent(car_day)
