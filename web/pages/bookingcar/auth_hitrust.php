...
if (!is_dev() && empty($orderId)) {
    writeLog('auth.hitrust od_id is empty ip:' . $_SERVER['REMOTE_ADDR'] . ' pid=' . $orderId);
...
// 取得訂單資料
$order_detail = $tripitta_service->get_car_order_by_order_id($orderId);
// if (is_dev()) printmsg('spend time:' . (microtime(true) - $t1));

if (empty($order_detail)) {
    writeLog('auth.hitrust order is empty ip:' . $_SERVER['REMOTE_ADDR'] . ' oo_id=' . $orderId);
    alertmsg('訂單資料錯誤!!', $error_url);
}

if ($order_detail['co_status'] != 10) {
    writeLog('訂單狀態錯誤[非授權前進入授權] oo_id=' . $orderId . ' co_status=' . $order_detail['co_status']);
    alertmsg('訂單狀態錯誤，尚未進行授權!!', $error_url);
}

// 進入授權，訂單金額不能為0
if ($order_detail['co_bank_trans_money'] <= 0) {
    writeLog('訂單金額錯誤 oo_id=' . $orderId . ' oo_bank_trans_money=' . $order_detail['co_bank_trans_money']);
    sendmail(get_config_mail_from(), Constants::$TRIPITTA_ADMIN_EMAILS, get_config_current_site() . 'Tripitta - hitrust auth訂單金額錯誤 oo_id=' . $orderId . ' oo_bank_trans_money=' . $order_detail['co_bank_trans_money'], json_encode($order_detail, JSON_UNESCAPED_UNICODE));
...
$curLang = 'tw';
$curDevice = 'computer';
if (!empty($order_detail['co_language'])) $curLang = $order_detail['co_language'];
if (!empty($order_detail['co_device'])) $curDevice = $order_detail['co_device'];
...
$transId = $order_detail['co_transaction_id'];
$amount = intval($order_detail['co_bank_trans_money']);
$bonus = $order_detail['co_bonus'];
...
if ('FAIL' == $ret['status'] || '000' != $ret['code']) {
    writeLog('無法與授權中心取得連繫 oo_id=' . $orderId . ' result:' . json_encode($ret, JSON_UNESCAPED_UNICODE));
    sendmail(get_config_mail_from(), Constants::$TRIPITTA_ADMIN_EMAILS, get_config_current_site() . 'Tripitta - hitrust auth授權返回狀態錯誤 oo_id=' . $orderId, json_encode($ret, JSON_UNESCAPED_UNICODE));
...
if ($returnCode == 10) {
    ...
    writeLog('授權中心的returnCode錯誤 oo_id=' . $orderId . ' returnCode=' . $returnCode . ' result:' . json_encode($xmlAry, JSON_UNESCAPED_UNICODE));
    sendmail(get_config_mail_from(), Constants::$TRIPITTA_ADMIN_EMAILS, get_config_current_site() . 'Tripitta - hitrust auth授權中心的returnCode錯誤 oo_id=' . $orderId . ' returnCode=' . $returnCode, json_encode($xmlAry, JSON_UNESCAPED_UNICODE));
    alertmsg('銀行回傳的授權資料錯誤!!', $error_url);
}
writeLog('hitrust auth oo_id=' . $orderId . ' oo_banking_status=' . $returnCode . ' action=' . $action);
