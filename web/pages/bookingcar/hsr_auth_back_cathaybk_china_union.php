...
// 取得訂單資料
$order_detail = $tripitta_service->get_ticket_order_by_trans_id($transactionId);
...
if ($returnCode == 10) {
    ...
    writeLog('授權中心的returnCode錯誤 to_id=' . $orderId . ' returnCode=' . $returnCode);
    sendmail(get_config_mail_from(), Constants::$TRIPITTA_ADMIN_EMAILS, get_config_current_site() . 'Tripitta - 銀聯卡 auth授權中心的returnCode錯誤 to_id=' . $orderId . ' returnCode=' . $returnCode);
    alertmsg('銀行回傳的授權資料錯誤!!', $error_url);
}
writeLog('銀聯卡 auth to_id=' . $orderId . ' to_banking_status=' . $returnCode . ' action=' . $action);
