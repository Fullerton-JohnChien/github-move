...
$form_url = "/bookingcar/charter/" . $fr_id . "/";
...
<head>
	...
	<title><?php ""; ?>交通預訂 - Tripitta 旅必達</title>
...
<body>
	...
	<main class="charter-intro-container">
				...
				<div class="detail">
					...
					<div class="cate">
						...
						<div class="cFrame">
							...
							<div class="ctitle">
								一日包車<span> <?php echo $fleet_route_detail['frd_category_value']; ?> 小時</span>
							</div>
							<div class="cstitle">
								( 基本時數 )
							</div>
					...
					<div class="imgWrap">
						...
						<?php
							...
							foreach ($car_array['photo'] as $value) {
								...
								$car_single_photo_check = "/photos/".$car."/car/".$c_id."/".$value['p_id'].".jpg";
								$car_single_photo = (is_file($car_single_photo_check)) ? $car_single_photo : "/web/img/no-pic.jpg";
						?>
		...
		<section>
			<div class="itine">
				...
				?>
				<div class="block <?php if ($value['crs_content_id'] != 0) { ?>pointed<?php } ?>">
					<?php if ($value['crs_content_id'] == 0) { ?>
					<div class="outer">
						<div class="circle">
							<div class="cont">
				...
				?>
				<div class="noted">
					以上建議行程僅供參考，實際行程及私房景點請與司機溝通，如超過基本範圍則須加價。
				</div>
			</div>
		</section>
		<section>
					...
					<div class="rBlock">
						...
						<div class="notice">
							<ul class="popupPolicy">
								<li>1.若有行程或車輛上的調動的問題，我們將於訂單成立後一個工作天內與您聯絡，如未
									能符合您的需求，我們將全額退費給您。</li>
								<li>2.最晚於出發日前24小時我們將會提供您車輛的車牌號碼、司機全名及手機號碼。</li>
								<li>3.半日包車基本時數：<span style="color:blue"><?=$half_day_hour[0]['frd_category_value']?></span>
									小時；一日包車基本時數：<span style="color:blue"><?=$full_day_hour[0]['frd_category_value']?></span>小時。</li>
								<li>4.超時，每小時加收NTD <span style="color:blue"><?=number_format($over_time_add_price[0]['frd_category_value'])?></span>元；
									<?for($i=0;$i<count($airport_midnight);$i++){?>
										...
										<?}?>
									<?}?>
									訂單價格不包含超時、深夜清晨及附加服務的加價費，請於現場付予司機。</li>
								<li>5.
								...
								<li>6.以上景點或使用時間及順序將有增減或調整之可能，實際以出車行程表為準。</li>
								<li>7.如遇不可抗力之因素，如颱風、地震、交通中斷等導致車行不能發車或您無法如期出發，將全額退款或依您意願，延後包車日期。</li>
								<li>8.本公司為代收代付平台(本服務恕無法開立發票)，若需求乘車收據，請務必於乘車時向司機索取，若當日未索取，視為棄權，無法補發重寄。</li>
							</ul>
						</div>
						<?}?>
						<div class="notice">
	...
	<script type="text/javascript">
            $(function () {
                ...
                $('#popupQuest .sBtnWrap #submit2').click(function () {
             	...
             	// 顯示車款介紹功能視窗
                $('.info .cate #carStyle').click(function () {
            		$('.overlay').show();
                    $('#popupCar').show();
            		$(window).scrollTop("0");
                });
                ...
                $('.submit3').click(function () {
                    var data = '<?php echo $header_is_login; ?>';
                    if(data == 1){
                    	location.href = "/web/pages/bookingcar/check_data.php?type=1&fr_id=<?= $fr_id ?>&begin_date=<?= $begin_date ?>&car_adult=<?= $car_adult ?>&car_child=<?= $car_child ?>";
                    }else{
                    	show_popup_login();
                    }
                });
