$(function(){
	...
	$('.improve-block .toggle-button').on('click', function(event) {
				
			if(_improveRightValue == -460)  $('.modal').remove();
			else  $('body').append('<div class="modal"></div>');

		$(this).parent('.improve-block').animate({
			right: _improveRightValue},
			300, function() {
				$('.improve-block .toggle-button i').toggleClass('fa-angle-left').toggleClass('fa-angle-right');
				_improveRightValue = (_improveRightValue == -460) ?  0 :  -460;
		});
	});
	...
	$(window).on('resize', function() {
		...
		for(var i=1; i < _finalNum; i++){
			$('.block-swiper .swiper-slide').eq(i).removeClass('opacity50').css({cursor: 'pointer'});			
		}
	...
	$('.arrow-left, .arrow-right').on('click' ,function(e){
		...
		switch(_id){
			case 'aLeft':
				...
				$list.prepend( $slide.eq(_finalNum-1).css('margin-left',  -_swiperSlideW).stop(true,true).animate({'margin-left':  0} , 500, 'easeOutSine'));
				$slide.eq(0).css('margin-left',  0);

				for(var i=0; i < _finalNum-1; i++){
					$slide.eq(i).removeClass('opacity50').css({cursor: 'pointer'});			
				}

			break;
			case 'aRight':
				$slide.eq(0).stop(true,true).animate({'margin-left': -_swiperSlideW}, 500, 'easeOutSine', function(){ $list.append($(this).css('margin-left', '')); });
